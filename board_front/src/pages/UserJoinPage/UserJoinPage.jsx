...
function UserJoinPage(props) {
    ...
    const sendMail = useMutation(
        async ({toEmail, username}) => await instance.post("/auth/mail", {toEmail, username}), // 매개변수 두개 이상은 객체, 배열이용
    )
    ...
    const handleJoinSubmitOnClick = async () => {
        ...
        const toEmail = signupData.ok.user.email; // 회원가입시 사용한 이메일
        const username = signupData.ok.user.username; 
        await sendMail.mutateAsync({toEmail, username}); // 회원가입할 때 바로하기
