...
function ScrollBoardListPage(props) {
    ...
    const boardList = useInfiniteQuery(
        ["boardScrollQuery"],
        async ({ pageParam = 1 }) => await instance.get(`/board/list?page=${pageParam}&limit=${limit}`), // pageParam이 null이거나 false면 요청 보내지 않음
        {
            getNextPageParam: (lastPage, allPage) => {
                ...
                return totalPageCount !== allPage.length ? allPage.length + 1 : null; // return값은 pageParam으로               
            }
        }
    );
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver((IntersectionObserver) => { 
            if(IntersectionObserver.isIntersecting) { // 스크롤을 내리다가 발견하면 isIntersecting이 true가 됨
    ...
    return (
        <div css={layout}>
            <Link to={"/"}><h1>I want to go home</h1></Link>
            <ul css={cardLayout}>
                {
